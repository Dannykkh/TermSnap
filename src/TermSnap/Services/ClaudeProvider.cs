...
public class ClaudeProvider : IAIProvider
{
    ...
    public ClaudeProvider(string apiKey, string model = "claude-3-5-sonnet-20241022")
    {
  ...
  ""related_commands"": [""관련 명령어1"", ""관련 명령어2""]
    ...
    private async Task<string> GenerateContent(string systemPrompt, string userPrompt)
    {
        try
        {
            ...
            {
                throw new Exception("Claude API로부터 유효한 응답을 받지 못했습니다.");
            }
        ...
        {
            throw new Exception($"Claude API 호출 실패: {ex.Message}", ex);
        }
    }
